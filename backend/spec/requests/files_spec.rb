...
describe 'FilesController', type: :request do
  ...
  context 'when user is authenticated' do
    before do
      ...
      UserFile.create!(user: user, file_name: 'testfile.txt', file_type: 'text/plain', file_size: 123, file_path: '/some/path')
    end
    ...
    it 'returns a list of files with metadata' do
      ...
      expect(json['files'].first).to have_key('id')
      expect(json['files'].first).to have_key('content_type')
      expect(json['files'].first).to have_key('file_size')
  ...
  describe 'GET /files/:file_id' do
    ...
    let!(:user_file) { UserFile.create!(user: user, file_name: 'testfile.txt', file_type: 'text/plain', file_size: 123, file_path: '/some/path') }
    let!(:other_file) { UserFile.create!(user: other_user, file_name: 'otherfile.txt', file_type: 'text/plain', file_size: 456, file_path: '/other/path') }
